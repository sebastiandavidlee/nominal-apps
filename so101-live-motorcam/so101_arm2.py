"""
SO-101 Robot Arm 2 Live Data Stream

This script connects to the second SO-101 robot arm via serial communication using 
the Feetech servo bus protocol. It continuously reads motor positions and streams 
them to the Nominal Connect application for real-time visualization.

Hardware: SO-101 Robot with Feetech servo motors (Arm 2)
...
# --- Configuration ---
SERIAL_PORT = "/dev/ttyACM0"  # Serial port for SO-101 Arm 2 connection (TESTING - try ACM1)
...
def main(connect_client: connect_python.Client):
    ...
    Connects to the SO-101 robot arm 2, reads motor positions at regular intervals,
    ...
    try:
        # Open serial connection to SO-101 Arm 2
        logger.info(f"Connecting to Arm 2 on {SERIAL_PORT} at {BAUD_RATE} baud...")
        ser = serial.Serial(SERIAL_PORT, BAUD_RATE, timeout=TIMEOUT)
        logger.info("Arm 2 connection established successfully.")
        
        # Clear any existing data in the stream
        connect_client.clear_stream("so101_arm2_motors")
        ...
        while True:
            ...
            # Log status every 10 iterations for debugging (more frequent)
            loop_count += 1
            if loop_count <= 5 or loop_count % 10 == 0:
                logger.info(f"Arm 2 Loop {loop_count}: Read {len(valid_positions)}/{len(MOTOR_IDS)} motors - Positions: {valid_positions}")
            ...
            if valid_positions:
                ...
                if loop_count <= 3:
                    logger.info(f"Arm 2: Successfully streamed to 'so101_arm2_motors'")
            else:
                if loop_count % 10 == 0:
                    logger.warning(f"Arm 2: No valid motor positions read at loop {loop_count}")
        ...
        logger.error("Check that Arm 2 is connected and not in use by another program.")
        
    except KeyboardInterrupt:
        logger.info("Arm 2 script stopped by user.")
        
    except Exception as e:
        logger.error(f"Arm 2 unexpected error: {e}")
        ...
        if ser and ser.is_open:
            ser.close()
            logger.info("Arm 2 serial connection closed.")
